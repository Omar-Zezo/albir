...
const VideosSections = () => {
  const [videosSectionData, setVideosSectionData] = useState(null);
        ...
        if (data.data.data) {
          if (data.data.data.sections) {
            if (data.data.data.sections.data) {
              setVideosSectionData(data.data.data.sections.data);
            }
          }
        }
      }
  ...
  return !videosSectionData ? (
    <PageLoader />
  ) : (
    <ScrollToTop>
      <Helmet>
        <title>جمعية البر - الفيديوهات</title>
      </Helmet>
            ...
            <div className="container pt-16">
              <div className="flex flex-col gap-4 pr-5">
                ...
                <div className="w-fit flex gap-2 text-base font-semibold text-white py-7 max-xl:py-5 px-5 pr-2 rounded-l-[27px] bg-mainColor">
                  ...
                  <span>-</span>
                  <p className="underline">الفيديو</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="flex flex-wrap justify-center gap-10 mt-5">
              {videosSectionData?.map((item) => (
                <div>
                  ...
                  <h4 className="pt-5 text-xl text-secondryColor hover:text-mainColor duration-300 cursor-pointer text-center font-medium">
                    {item?.title}
                  </h4>
                </div>
              ))}
