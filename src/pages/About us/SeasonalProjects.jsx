...
import { getSeasonalProjects } from "../../store/slices/Home/SeasonalProjects";
...
const SeasonalProjects = () => {
      ...
      if (data.data) {
        if (data.data.data) {
          if (data.data.data.projects) {
            if (data.data.data.projects.data) {
  ...
  return !projects ? (
    ...
    <ScrollToTop>
      <Helmet>
        <title>جمعية البر - المشاريع الموسمية</title>
      </Helmet>
            ...
            <div className="container pt-16">
              <div className="flex flex-col gap-4 pr-5">
                ...
                <div className="w-fit flex gap-2 text-base font-semibold text-white py-7 max-xl:py-5 px-5 pr-2 rounded-l-[27px] bg-mainColor">
                  ...
                  <span>-</span>
                  <p className="underline">المشاريع الموسمية</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="container flex flex-col gap-4">
            <div className="flex flex-wrap justify-center gap-10 mt-20">
              {projects?.map((project) => (
                <SeasonalProjectCard key={project.id} project={project} />
              ))}
              <div />
            </div>
          </div>
